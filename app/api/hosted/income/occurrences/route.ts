...
export async function GET(request: Request) {
  try {
    return await createHostedIncomeRouteRuntime(await getHostedBindings()).occurrences.GET(request);
  } catch {
...
export async function POST(request: Request) {
  try {
    return await createHostedIncomeRouteRuntime(await getHostedBindings()).occurrences.POST(request);
  } catch {
    return Response.json({ error: "Hosted runtime is unavailable." }, { status: 500, headers: { "Cache-Control": "no-store" } });
  }
}
